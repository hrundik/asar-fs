...
asar.disk = require('asar/lib/disk');

// Overriding `searchNodeFromDirectory` to return undefined if file doesn't exist in archive
// Otherwise it just gives RTE on unexisting path, while monkey patch expects null value
function patchAsarFS(archFs) {
    _archFs.searchNodeFromDirectory = function (p) {
      let dir, dirs, json, _i, _len;
      json = this.header;
      dirs = p.split(path.sep);
      for (_i = 0, _len = dirs.length; json && _i < _len; _i++) {
        dir = dirs[_i];
        if (dir !== '.') {
          json = json.files[dir];
        }
    ...
    _findPath: function _findPath(request, paths) {
      ...
      ret = paths.reduce(function (acc, testPath) {
        if (!acc) {
          ...
          if (ASAR_PATH.test(relPath)) {
            relPath = relPath.split(ASAR_PATH)[1];
          }
